...
class WGAN():
    ...
    def __init__(self, input_nc, output_nc, seqL=100, lr=1e-4, gpu_ids='0', l1_w=10):
        ...
        self.optim_g = torch.optim.RMSprop(self.generator.parameters(), lr=lr)
        self.optim_d = torch.optim.RMSprop(self.discriminator.parameters(), lr=lr)
...
def main():
    parser = argparse.ArgumentParser(description="Training CGAN.")
    parser.add_argument('-name', type=str, help='name of data', default='human_enhancer_hepG2_cage_mask_200')
    parser.add_argument('-seqL', type=int, help='lenth of seq', default='200')
    parser.add_argument('-gpu', type=int, help='lenth of seq', default='0')
    ...
    data_name = args.name
    seqL = args.seqL
    train_data, test_data = DataLoader(LoadData(is_train=True, path='../data/generator_data/{}.csv'.format(data_name), split_r=1),
                                       batch_size=32, shuffle=True), DataLoader(LoadData(is_train=True, path='../data/generator_data/{}.csv'.format(data_name), split_r=1), batch_size=32)
    train_data = get_infinite_batches(train_data)
    logger = gen_utils.get_logger(log_path='cache/training_log/', name=data_name)
    n_critics = 5
    n_iters = 10000
    model = WGAN(input_nc=4, output_nc=4, seqL=seqL,gpu_ids=str(args.gpu), l1_w=50,lr=5e-5)
    ...
    for i in range(n_iters):
        ...
        if i % 100 == 99:
            ...
            A_dict_valid, A_dict_ref, T_dict_valid, T_dict_ref = gen_utils.polyAT_freq(csv_name, '../data/generator_data/{}.csv'.format(data_name))
            ...
            logger.info('polyT ref TTTTT:{} TTTTTT:{} TTTTTTT:{} TTTTTTTT:{}'.format(T_dict_ref['TTTTT'],
                                                                               ...
                                                                               T_dict_ref['TTTTTTT'],
                                                                               T_dict_ref['TTTTTTTT']))
            gen_utils.kmer_frequency(csv_name, '../data/generator_data/{}.csv'.format(data_name), k=4, save_path='cache/figure/', save_name=data_name + str(i))
            csv2fasta(csv_name, 'cache/gen_', 'iter_{}'.format(i))
            torch.save(model.generator, f'check_points/{data_name}_net_G_{i}.pth')
            torch.save(model.discriminator, f'check_points/{data_name}_net_D_{i}.pth')
